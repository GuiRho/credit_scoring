...
if __name__ == "__main__":
    ...
    # 9. Generate and Save SHAP Dependency Plots (Individual plots for each of the top 8 features)
    logger.info("Generating individual SHAP Dependency Plots for the top 8 features for all classes...")
    try:
        # Create a subdirectory for dependency plots for better organization
        dependency_plots_dir = os.path.join(output_dir, "dependency_plots")
        os.makedirs(dependency_plots_dir, exist_ok=True)

        for class_idx in range(n_classes):
            class_name = class_names[class_idx]
            logger.info(f"Generating SHAP dependency plots for class: {class_name}")
            
            # Create a class-specific subdirectory for dependency plots
            class_dependency_dir = os.path.join(dependency_plots_dir, f"class_{class_name.replace(' ', '_').lower()}")
            os.makedirs(class_dependency_dir, exist_ok=True)

            current_shap_values = shap_values_sample1[class_idx] # Using Sample 1 for dependency plots

            for i, feature in enumerate(top_8_feature_list):
                try:
                    logger.info(f"  Plotting feature: '{feature}' (feature {i+1} of {len(top_8_feature_list)}) for class: '{class_name}'")
                    
                    plt.figure(figsize=(10, 8)) # New figure for each plot
                    
                    shap.dependence_plot(
                        feature,
                        current_shap_values,
                        X_sample1_processed_for_shap,
                        display_features=X_sample1_processed_for_shap, # Use X_processed for color if needed
                        show=False # Crucial for saving
                    )
                    
                    plt.title(f'SHAP Dependence Plot: {feature} (Class: {class_name})', fontsize=14)
                    plt.tight_layout() # Adjust layout for current figure
                    
                    # Save the individual plot
                    sanitized_feature_name = feature.replace('/', '_').replace('\\', '_') # Sanitize for filename
                    dependency_plot_path = os.path.join(class_dependency_dir, f"shap_dependency_{sanitized_feature_name}_class_{class_name.replace(' ', '_').lower()}.png")
                    plt.savefig(dependency_plot_path, dpi=PLOT_DPI, bbox_inches='tight')
                    plt.close() # Close the figure to free memory
                    logger.info(f"  SHAP Dependency Plot for '{feature}' (Class {class_name}) saved to {dependency_plot_path}")

                except Exception as e:
                    logger.error(f"  FAILED to generate dependency plot for feature '{feature}' (class {class_name}). Error: {e}")
                    plt.close() # Ensure figure is closed even on error
                    continue # Continue to the next feature if one fails
