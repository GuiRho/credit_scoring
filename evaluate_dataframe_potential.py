...
def evaluate_dataframe(df_or_path, target_col: str = "TARGET", test_size: float = 0.2,
    ...
    persist_parquet: if True, balanced parquet files will be written to cache_dir.
    Returns a dict with per-balance metrics and the balanced DataFrames under 'balanced_dfs'.
    """
    ...
    for name, frac in balances.items():
        ...
        debug_fp = os.path.join(cache_dir, f"df_eval_debug_{Path(path_parquet).stem}_{name}.json")
        debug_info = {
